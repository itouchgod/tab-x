...
- Search text typed into the Tab X search box so it can open URLs or send searches to Chrome's default search provider. When at least two non-whitespace characters are typed, the current text is also sent to Google's suggestion endpoint (`suggestqueries.google.com`) to provide live autocomplete suggestions.
...
Tab X does not operate a server, does not create an account, and does not sell user data. Search-box text is sent to Google's suggestion service while typing as described above; Google processes those requests under its own privacy practices.
